...
monthly_subscriptions AS (
    ...
    FROM
        {{ unit_testing_select_table(ref('dim_subscriptions'),ref('unit_test_input_dim_subscription')) }}
    WHERE
...
months AS (
    ...
    FROM
        {{ unit_testing_select_table(ref('int_dates'),ref('unit_test_input_int_dates')) }}
    WHERE
        day_of_month = 1
),
...
subscription_periods AS (
    SELECT
        ...
        CASE
            ...
            WHEN end_month IS NULL THEN DATE(DATEADD('month', 1, {{ month_trunc('CURRENT_DATE') }} ))
            ELSE end_month
        END AS end_month
    FROM
